...
function initApps(callback) {
  ...
  try {
    ...
    _.each(N.config.applications, function (name) {
      var
      appRoot     = path.dirname(require.resolve(name)),
      appConfig   = require(name);

      N.runtime.apps.push(new Application(appRoot, appConfig));
    });
...
module.exports = function (callback) {
  ...
  async.series([initConfig, initLogger, initApps], callback);
};
