...
function initLogger(N, callback) {
  ...
  _.each(config, function (loggerConfig, name) {
    ...
    if (!appenders[loggerConfig.file]) {
      filename = path.resolve(mainRoot, loggerConfig.file);

      // make sure destination directory for log file exists
      fstools.mkdirSync(path.dirname(filename));

      appenders[loggerConfig.file] = log4js.appenders.file(
        filename,                             // filename
        null,                                 // layout
    ...
    // get "raw" appender (without thresholding)
    appender = appenders[loggerConfig.file];

    // prepare thresholded appender
    log4js.addAppender(thresholdedAppender(loggerConfig.level, appender), name);
