...
//  resolveApiPath()
//
//  Default API path resolver.
//
function resolveApiPath(root, filename) {
  var api = apify(filename, root, path.extname(filename));

  // deduplicate api path parts:
...
//  - *resolveApiPath(root, file)* (Function, Optional)
//
//
//  ##### Results
//
...
module.exports = function findPaths(options, callback) {
  ...
  var api_prefix  = '';
  var api_resolve = options.resolveApiPath || resolveApiPath;
  var pathnames   = [];
  ...
  fstools.walk(root, function (file, stats, next) {
    var api_path  = api_prefix + api_resolve(root, file);
    ...
    pathnames.push(new Pathname(api_path, relative, file));
    next();
  }, function (err) {
    if (err) {
      callback(err);
