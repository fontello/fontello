...
const beforeWithRuntime = {
  ...
  '.hbs':  _.template("(function () { N.views['<%= apiPath %>'] = require('handlebars/runtime').template(")
};

const afterWithRuntime = {
  '.hbs': ')})();'
};
...
module.exports = function (sandbox) {
  _.forEach(sandbox.config.packages, (pkg, pkg_name) => {
    _.forEach(pkg.files.widget_view, file_info => {
      ...
      let wrapAfter  = afterWithRuntime[ext]  ? afterWithRuntime[ext]  : after;
