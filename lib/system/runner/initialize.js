...
// Returns an object with keys: `responder` (string) and `method` (array).
// Each one may be `null` which means 'any'.
//
//   'rpc@'             => { responder: 'rpc',  method: null }
//   'http@forum.index' => { responder: 'http', method: [ 'forum', 'index' ] }
//   'blogs'            => { responder: null,   method: [ 'blogs' ] }
//
function parseLoggerName(name) {
  var responder, method, parts = name.split('@');

  if (1 === parts.length) {
    responder = null;
    method    = _.compact(name.split('.'));

  } else if (2 === parts.length) {
    responder = parts[0];
    method    = _.compact(parts[1].split('.'));
  ...
  return { responder: responder, method: method };
}
...
function initLogger(N) {
  ...
  N.logger.getLogger = function (name) {
    ...
    var chosenLogger
      , parsedName = parseLoggerName(name);
    ...
    chosenLogger = _.find(loggers, function (logger) {
      var testName = logger.parsedName;

      if (parsedName.responder && testName.responder &&
          parsedName.responder !== testName.responder) {
        return false;
      }

      if (parsedName.method && testName.method) {
        if (parsedName.method.length < testName.method.length) {
          return false;
        }

        return _.every(testName.method, function (part, index) {
          return part === parsedName.method[index];
        });
      } else {
        return true;
      }
  ...
  _.each(config, function (loggerConfig, name) {
    ...
    if ('system' !== name) {
      ...
      resultLogger.parsedName = parseLoggerName(name);
  ...
  loggers.sort(function (a, b) {
    if (a.parsedName.method && b.parsedName.method) {
      // Both loggers have a specified method.

      if (a.parsedName.method.length < b.parsedName.method.length) {
        return 1;

      } else if (a.parsedName.method.length > b.parsedName.method.length) {
        return -1;

      } else {
        // Both loggers have the same method length.

        if (a.parsedName.responder && b.parsedName.responder) {
          // Both loggers have a specified responder.
          return 0;
        } else {
          // Logger which has a responder is more specific.
          return a.parsedName.responder ? -1 : 1;
        }
      }
    } else {
      // Logger which has a method is more specific.
      return a.parsedName.method ? -1 : 1;
    }
  });
}
