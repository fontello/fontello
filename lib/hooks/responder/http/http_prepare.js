...
// Returns request type for server method selection.
// It should be used like so: route.meta.methods[type]
//
function getRequestType(req) {
  var type = req.method.toLowerCase();

  return ('head' === type) ? 'get' : type;
}
...
module.exports = function (N) {
  ...
  N.wire.before('responder:http', function http_prepare(env, callback) {
    ...
    if (!_.has(match.meta.methods, type)) {
      env.err = N.io.NOT_FOUND;
      callback();
      return;
    }

    env.method = match.meta.methods[type];
