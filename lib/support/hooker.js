...
var SortedList  = require('types').SortedList;
...
function get_hooks(self, bucket) {
  if (!self.__hooks__[bucket]) {
    self.__hooks__[bucket] = {
      before: SortedList.create(),
      after:  SortedList.create(),
      ensure: SortedList.create()
    };
  }
...
function get_sorted_hooks(self, bucket) {
  ...
  if (!self.__cache__[bucket]) {
    ...
    while (parts.length) {
      ...
      before  = hooks.before.concat(before);
    ...
    self.__cache__[bucket] = {
      before: get_filtered_funcs(bucket, before.sorted),
      after:  get_filtered_funcs(bucket, after.sorted),
      ensure: get_filtered_funcs(bucket, ensure.sorted)
    };
  }
...
function filter(self, chain, bucket, options, fn) {
  ...
  hooks[chain].add(weight, {exclude: exclude, func: fn});
  return true;
}
