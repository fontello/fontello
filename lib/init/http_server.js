...
function attach_connect_app(server) {
  ...
  static_helpers.count_online_users = function () {
    try {
      return nodeca.runtime.sio.sockets.clients().length + 1;
    } catch (err) {
      nodeca.logger.error("Failed get list of socket.io clients: " +
...
function attach_socket_io(server) {
  ...
  sio.sockets.on('connection', function (socket) {
    socket.broadcast.emit('users_online', sio.sockets.clients().length);

    socket.on('disconnect', function () {
      this.broadcast.emit('users_online', sio.sockets.clients().length - 1);
    });
