...
http.attach = function attach(server, next) {
  var parse_query = connect.query();

  // fix host
  nodeca.filters.before('', function fix_vhost(params, callback) {
  ...
  // parse query params
  nodeca.filters.before('', function parse_query_params(params, callback) {
    var http = this.origin.http, host;
    ...
    parse_query(http.req, http.res, callback);
  });
