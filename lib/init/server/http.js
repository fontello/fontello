...
function process_error(req, res, err) {
  var body = err.body, head = _.extend({}, err.headers);

  if (err.redirect) {
    head = _.extend(head, {'Location': err.redirect});
    end(req, res, err.statusCode || 302, head, body);
    return;
  }

  if (err.denied) {
    if (!body) {
      head = _.extend(head, {'Content-Type': 'text/plain'});
      body = 'Forbidden';
    }
    end(req, res, 403, head, body);
    return;
  }

  //
  // custom error / request termination
  //

  if (err.statusCode) {
    end(req, res, err.statusCode, head, body);
    return;
  }
