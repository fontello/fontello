...
function process_error(req, res, err) {
  ...
  if (err.denied) {
    ...
    end(res, 403, {'Content-Type': 'text/plain'}, 'Forbidden');
    return;
  }
  ...
  end(res, 500, {'Content-Type': 'text/plain'}, body);
}
...
http.attach = function attach(server, next) {
  ...
  // render view script
  nodeca.filters.after('', {weight: 100}, require('./http/view_renderer'));

  // render view script
  nodeca.filters.after('', {weight: 100}, require('./http/compressable'));

  // render view script
  nodeca.filters.after('', {weight: 100}, require('./http/cachable'));
