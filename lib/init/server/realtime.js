...
realtime.attach = function attach(server, next) {
  var faye = new Faye.NodeAdapter({mount: '/faye', ping: 20, timeout: 30});
  ...
  //
  // Log Faye's messages to separate file
  //

  log4js.addAppender(log4js.appenders.file(
    require('path').resolve(__dirname, '../../../log/faye.log')
  ), 'faye');
