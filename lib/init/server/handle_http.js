...
function end(req, res, code, head, body, env, err) {
  ...
  //
  // Unset body if we are handling HEAD request
  //
...
module.exports = function handle_http(req, res) {
  ...
  // test() returns `null` when method has no validation schema,
  // object with `valid` property otherwise
  if (!validation) {
  ...
  func = treeGet(N.server, match.meta);
