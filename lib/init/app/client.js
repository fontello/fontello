...
Requisite.prototype.bundle = function () {
  ...
  parts.push(
    '(function () {',
      'var modules = {};',
      'function define(name, api, init) {',
        'modules[name] = { api: api, init: init };',
      '}',
      'function require(name) {',
        ...
        'if (!modules[name].module) {',
          'function t(phrase, params) {',
            ...
            '} else {',
              'phrase = modules[name].api + "." + phrase;',
            '}',
  ...
  _.each(this.requires, function (dep, pathname) {
    parts.push(
      'define(' + dep.idx + ', ' + JSON.stringify(dep.api) + ', function (exports, module, require, t, define, modules) {',
      dep.source,
      '});'
    );
  });
