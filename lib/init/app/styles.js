...
function compileStyles(pkgName, tmpdir, config, callback) {
  var
  assetsOutdir = path.join(tmpdir, pkgName, 'bin'),
  stylesOutdir = path.join(tmpdir, pkgName, 'styles'),
  stylesFile   = path.join(stylesOutdir, 'styles.css'),
  mainFile     = stylesFile;

  writeRenderedStyles(stylesFile, config, function (err) {
    ...
    environment.appendPath(stylesOutdir);
    ...
    environment.findAsset(mainFile).compile(function (err, data) {
      var outfile = path.join(tmpdir, pkgName, pkgName + '.css');
